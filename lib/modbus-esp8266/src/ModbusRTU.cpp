...
{
	const uint32_t baud_default = 38400;

	/*fixed from static code analysis*/
	if (baud == 0)
	{
		baud = baud_default;
	}
...
/**
 * @brief  Function to disable the serial
 * @param  none
 * @retval none
 */
void Disable_serial_with_tx_activated(void)
{
	Serial2.end();
	pinMode(17, INPUT_PULLDOWN);
}
/**
 * @brief  Function to enable the serial with rx and tx active
 * @param  baud_rate
 * @param  rx_pin
 * @param  tx_pin
 * @retval none
 */
void Enable_serial_with_tx_activated(unsigned long baud_rate, int8_t rx_pin, int8_t tx_pin)
{
 ...
 * @retval none
 */
void Enable_serial_with_tx_not_activated(unsigned long baud_rate, int8_t rx_pin)
{
